...

{
    ...
    public function delete($id, Request $request, Planning $planning, PlanningRepository $planningRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$planning->getId(), $request->request->get('_token'))) {
            $planningRepository->remove($planning, true);
        }

        return $this->redirectToRoute('app_planning_user', ['id'=>$this->getUser()->getId()]);
    }
    ...
    #[Route('/reservation/user/{id}', name: 'app_planning_user', methods: ['GET'])]
    public function myReservation($id, UserRepository $userRepository, PlanningRepository $planningRepository): Response
    {
        ...
        return $this->render('planning/myReservation.html.twig', [
            'plannings' => $planningRepository->findOrderDate($user),
        ]);
    }
